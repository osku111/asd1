  ...
  return (
    <Layout>
      <div className="space-y-8">
        {/* Header */}
        <div>
          <h1 className="text-4xl font-bold text-foreground mb-2">GPS Tracking Dashboard</h1>
          <p className="text-muted-foreground">
            Real-time monitoring of all tracked devices and their locations
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 h-96 lg:h-[500px] rounded-lg border border-border overflow-hidden bg-card shadow-sm">
            <MapView devices={devices} selectedDevice={selectedDevice} routes={routes} />
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 border-b border-border">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-medium text-muted-foreground">Speed</th>
                  <th className="px-6 py-3 text-left font-medium text-muted-foreground">Location</th>
                  <th className="px-6 py-3 text-left font-medium text-muted-foreground">Last Update</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {devices.map((device) => (
                  <tr key={device.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4 text-foreground">{device.name}</td>
